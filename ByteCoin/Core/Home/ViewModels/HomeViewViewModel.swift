...
class HomeViewViewModel : ObservableObject {
    ...
    func getCoinsForTopGainerAndLoserList () async {
        ...
        case .failure(let error):
            print("List of Top Gainers and Top Losers Could Not Be Retrieved : \(error)")
        }
    ...
    func getCoinsWithPagination () async {
        ...
        case .failure(let error):
            print("Paging could not be achieved : \(error)")
        }
    ...
    func getPortfolioCoins (userId : String) async {
        ...
        do {
            ...
            print("Portfolio could not be received :\(error)")
        }
    ...
    func getWatchListCoins (userId : String) async {
        ...
        do {
            ...
            print("Watch list could not be received :\(error)")
        }
    ...
    func getAndUpdateCoin (userId : String, coin : AllCoinsDataResponseModel) async {
        ...
        do {
            ...
            print("Could not retrieve data from Firebase :\(error)")
        }
        
    }
    
    func setNewWatchListCoin (userId : String, coin: AllCoinsDataResponseModel) async {
        ...
        if watchListCoins.contains(where: { $0.id == coin.id}) || portfolioCoins.contains(where: { $0.id == coin.id}) {
            if let isInWatchList = coin.isInWatchList {
                if coin.currentCoinAmount == 0 && isInWatchList {
                    do {
                        ...
                        print("Failed to delete coin from watchlist : \(error)")
                    }
                }else {
                    do {
                        ...
                        print("Coin watchlist could not be updated : \(error)")
                    }
                }
            }
        }else {
            do {
                ...
                print("Coin could not be registered in the watch list : \(error)")
            }
        }
    ...
    func buyCoin (userId : String, coin : AllCoinsDataResponseModel, newCoinAmount : Double) async {
        ...
        if portfolioCoins.contains(where: {$0.id == coin.id}) || watchListCoins.contains(where: { $0.id == coin.id}) {
            ...
            if var currentAmount = coin.currentCoinAmount {
                ...
                do {
                    ...
                    print("Failed to update the amount of coins bought : \(error)")
                }
            }
        }else {
            do {
                ...
                print("Failed to register the amount of coins bought : \(error)")
            }
        }

    }
    
    func sellCoin (userId : String, coin : AllCoinsDataResponseModel, newCoinAmount : Double) async {
            ...
            if currentAmount <= 0 {
                if !(coin.isInWatchList ?? true ) {
                    do {
                        ...
                        print("Failed to delete the amount of coins sold : \(error)")
                    }
                }else {
                    do {
                        try await databaseManager.updateData(
                        ...
                        print("Failed to update the amount of coins sold : \(error)")
                    }
                }
            }else {
                do {
                    ...
                    print("Failed to register the amount of coins sold : \(error)")
                }
            }
        }
    }
...
extension HomeViewViewModel {
    ...
    private func getCoinsWithIds (ids : [String]) async -> [AllCoinsDataResponseModel]{
        ...
        case .success(let coins):
            return coins
        case .failure(let error):
            print("Failed to get coins matching IDs : \(error)")
        }
