...
struct CoinExchangeButtonView: View {
    ...
    var body: some View {
        BuyOrSellButtonItem(isButtonDisabled: buyOrSellButtonDisabled(), buttonType: buttonType) {
            ...
            case .buy:
                Task {
                    await homeViewModel.buyCoin(userId : launchViewModel.userId, coin: coin, newCoinAmount: coinAmount.convertToDouble())
                }
            case .sell:
                Task {
                    await homeViewModel.sellCoin(userId : launchViewModel.userId, coin: coin, newCoinAmount: coinAmount.convertToDouble())
                }
            }
